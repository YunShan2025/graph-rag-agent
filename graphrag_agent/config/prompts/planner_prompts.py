...
**可用任务类型**:
1. **local_search**: 在知识图谱中检索特定实体的详细信息和局部关系（微观视角，针对具体实体）。**适用于：查询具体数据（金额、比例、日期、条件等）、实体详情、具体规定**
2. **global_search**: 在知识图谱中检索整体概念和社区级摘要信息（宏观视角，针对主题概念）。**适用于：需要全局概览、跨社区比较、主题趋势分析**
...
**工具选择关键规则**:
- **简单事实查询**（问具体数据、条件、规定）→ 优先用 **local_search**
- **复杂分析查询**（问原因、趋势、比较、综合）→ 可用 **global_search** 或 **deep_research**
- **不确定时**，默认选择 **local_search**（它能检索到具体文档内容）
...
```
注意：这类问具体数据的查询，必须用 local_search，不要用 global_search。
...
**再次强调工具选择规则**:
- 问"是什么/多少/哪些/条件"这类具体事实 → **local_search**
- 问"为什么/如何/分析/比较"这类复杂问题 → **global_search** 或 **deep_research**
- 简单查询只需1个任务，不要拆分过多
