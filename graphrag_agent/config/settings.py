...
lc_description = (
    "用于需要查找具体事实、数据、条款的查询。检索中南大学学生管理文件中的原文内容，包括："
    "金额数字（如奖学金多少钱）、具体比例（如获奖比例）、具体条件（如申请要求）、"
    "具体流程（如评审步骤）、具体规定（如请假天数、处分标准）等。"
    "适用于问'多少'、'几天'、'什么条件'、'具体要求是什么'等问题。"
    "注意：涉及具体数字、金额、比例、天数的问题必须使用此工具。"
)
gl_description = (
    "用于需要跨多个主题进行总结归纳、对比分析的宏观查询。"
    "分析中南大学学生管理体系的整体框架、不同政策之间的关系、多类奖学金的横向对比等。"
    "适用于'学校有哪些资助政策'、'各类奖学金有什么区别'、'学生管理体系的整体思路'等需要系统性分析的问题。"
    "注意：不要用于查询具体数字、金额、比例等细节问题。"
)
naive_description = (
    "基础检索工具，直接查找与问题最相关的文本片段，不做复杂分析。快速获取中南大学相关政策，返回最匹配的原文段落。"
)
